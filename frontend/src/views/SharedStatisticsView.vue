                ...
                <v-window v-model="currentTab">
                  ...
                  >
                    ...
                    <div v-if="mode === 'DASHBOARD' && processedStats['DASHBOARD']">
                      ...
                      <!-- 対戦履歴 -->
                      <v-card class="duel-card mt-4" v-if="(processedStats['DASHBOARD'] as any).duels && (processedStats['DASHBOARD'] as any).duels.length > 0">
                      ...
                      <v-row>
                          ...
                          <v-col cols="12" md="6">
                            <v-card class="stats-card">
                              ...
                              <v-card-text>
                                <apexchart
                                  v-if="processedStats[mode].monthlyDistribution && processedStats[mode].monthlyDistribution.series && processedStats[mode].monthlyDistribution.series.length > 0"
                                  type="pie"
                                  height="350"
                    ...
                    <v-col cols="12" md="6">
                      <v-card class="stats-card">
                        ...
                        <v-card-text>
                          <apexchart
                            v-if="processedStats[mode].recentDistribution && processedStats[mode].recentDistribution.series && processedStats[mode].recentDistribution.series.length > 0"
                            type="pie"
                            height="350"
                    ...
                    <v-col cols="12">
                      <v-card class="stats-card">
                        ...
                        <v-card-text>
                          <div v-if="processedStats[mode].matchupData && processedStats[mode].matchupData.length > 0">
                            <v-data-table
                    ...
                    <v-col cols="12" v-if="mode === 'RATE' || mode === 'DC'">
                      <v-card class="stats-card">
                        <v-card-title>{{ mode === 'RATE' ? 'レート変動' : 'DC変動' }}</v-card-title>
                        <v-card-text>
                          <apexchart
                            v-if="processedStats[mode].timeSeries && processedStats[mode].timeSeries.series && processedStats[mode].timeSeries.series[0] && processedStats[mode].timeSeries.series[0].data && processedStats[mode].timeSeries.series[0].data.length > 0"
                            type="line"
                            height="350"
...
const getGameModeIcon = (modeName: string) => {
  switch (modeName) {
    ...
    case 'STATISTICS': return 'mdi-chart-box-outline'
    case 'RANK': return 'mdi-crown'
    case 'RATE': return 'mdi-chart-line'
    case 'EVENT': return 'mdi-calendar-star'
    case 'DC': return 'mdi-trophy-variant'
        ...
        if (mode === 'DASHBOARD') {
          ...
          tempProcessedStats['STATISTICS'] = {
            ...
            monthlyDistribution: {
              ...
              chartOptions: { ...baseChartOptions, labels: rawStats.monthly_deck_distribution?.map((d: any) => d.deck_name) || [] },
            },
            recentDistribution: {
              series: rawStats.recent_deck_distribution?.map((d: any) => d.count) || [],
              chartOptions: { ...baseChartOptions, labels: rawStats.recent_deck_distribution?.map((d: any) => d.deck_name) || [] },
            },
          ...
          tempProcessedStats[mode] = {
            ...
            matchupData: rawStats.matchup_data || [],
            timeSeries: {
              series: [{ name: mode, data: rawStats.time_series_data?.map((d: any) => d.value) || [] }],
              chartOptions: {
                ...lineChartBaseOptions,
                xaxis: { ...lineChartBaseOptions.xaxis, categories: rawStats.time_series_data?.map((_: any, i: number) => i + 1) || [] },
