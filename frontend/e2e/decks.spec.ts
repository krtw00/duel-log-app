...
test.describe('Deck Management', () => {
  ...
  test('should display deck list', async ({ page }) => {
    ...
    const deckListVisible = await deckList.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('should create a new deck', async ({ page }) => {
    ...
    const deckTypeInput = page.locator('input[name="deckType"], select[name="deckType"]');
  ...
  test('should edit a deck', async ({ page }) => {
    ...
    // デッキが作成されるまで待つ
    await expect(page.getByText(originalDeckName, { exact: false })).toBeVisible({ timeout: 5000 });
    ...
    if (await editButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      // デッキ名を変更
      const editDeckNameInput = page.locator('input[name="name"], input[name="deckName"]');
      ...
      await expect(page.getByText(newDeckName, { exact: false })).toBeVisible({ timeout: 5000 });
    } else {
      test.skip();
    }
  });
  ...
  test('should delete a deck', async ({ page }) => {
    ...
    if (await deleteButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      await expect(page.getByText(deckName, { exact: false })).not.toBeVisible({ timeout: 5000 });
    } else {
      test.skip();
    }
  });
