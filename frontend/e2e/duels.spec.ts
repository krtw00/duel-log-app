...
test.describe('Duel Management', () => {
  ...
  test('should display dashboard with duel history section', async ({
    page,
  }) => {
    await page.goto('/')
    ...
    const duelHistory = page.locator(
      '.duel-history, [data-testid="duel-history"]'
    )
    const emptyState = page.getByText(
      /対戦履歴がありません|No duels|対戦を記録/i
    )

    const duelHistoryVisible = await duelHistory
  ...
  test('should open duel creation form', async ({ page }) => {
    ...
    if (
      await addDuelButton.isVisible({ timeout: 5000 }).catch(() => false)
    ) {
      await addDuelButton.click()

      // フォームまたはダイアログが表示されることを確認
      const duelForm = page.locator(
        'form, .duel-form, [data-testid="duel-form"], .v-dialog'
      )
  ...
  test('should create a new duel record', async ({ page }) => {
    ...
    if (
      await createDeckButton.isVisible({ timeout: 5000 }).catch(() => false)
    ) {
    ...
    if (
      ...
      if (await winButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        await loseButton.isVisible({ timeout: 3000 }).catch(() => false)
      ) {
      ...
      if (
        await opponentDeckInput.isVisible({ timeout: 3000 }).catch(() => false)
      ) {
      ...
      if (
        await saveDuelButton.isVisible({ timeout: 3000 }).catch(() => false)
      ) {
        await saveDuelButton.click()

        // 対戦記録が保存されたことを確認（成功メッセージまたはリストに追加）
        const successMessage = page.locator(
          '.v-snackbar, .success-message, [role="alert"]'
        )
  ...
  test('should view statistics page', async ({ page }) => {
    ...
    if (await statsLink.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const statsContent = page.locator(
        '.statistics, [data-testid="statistics"], main'
      )
