...
--
-- This migration is historical. Fresh databases already start from the new
-- UUID-based schema in 20240101000000_initial_schema.sql, so we skip the body
-- unless a legacy column (`supabase_uuid`) is still present.
  ...
  IF EXISTS (
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        ss.game_mode,
        'from',
        make_timestamptz(ss.year, ss.month, 1, 0, 0, 0)::text,
        'to',
        (make_timestamptz(ss.year, ss.month, 1, 0, 0, 0) + interval '1 month' - interval '1 second')::text
      ),
      ss.expires_at,
    ...
    WHERE EXISTS (
      SELECT 1
      FROM generic_deck_patterns p
      WHERE lower(decks.name) LIKE '%' || lower(p.pattern) || '%'
    );
  ELSE
    RAISE NOTICE 'Skipping legacy schema migration; database already uses UUID schema.';
  END IF;
END
$$;
