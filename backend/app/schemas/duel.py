...
class DuelBase(CustomBaseModel):
    ...
    opponent_deck_id: int = Field(..., gt=0, description="対戦相手デッキID", validation_alias="opponentDeck_id", serialization_alias="opponentDeck_id")
    won_coin_toss: bool = Field(..., description="コイントスの結果（True: 表, False: 裏）", validation_alias="coin", serialization_alias="coin")
    is_going_first: bool = Field(
        ..., description="先攻後政（True: 先攻, False: 後攻）", validation_alias="first_or_second", serialization_alias="first_or_second"
    )
    is_win: bool = Field(..., description="対戦結果（True: 勝利, False: 敗北）", validation_alias="result", serialization_alias="result")
...
class DuelUpdate(BaseModel):
    ...
    opponent_deck_id: Optional[int] = Field(None, gt=0, description="対戦相手デッキID", validation_alias="opponentDeck_id", serialization_alias="opponentDeck_id")
    won_coin_toss: Optional[bool] = Field(None, description="コイントスの結果", validation_alias="coin", serialization_alias="coin")
    is_going_first: Optional[bool] = Field(None, description="先攻後攻", validation_alias="first_or_second", serialization_alias="first_or_second")
    is_win: Optional[bool] = Field(None, description="対戦結果", validation_alias="result", serialization_alias="result")
