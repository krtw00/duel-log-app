...
class GeneralStatsService:
    ...
    def _calculate_general_stats(self, duels: List[Duel]) -> Dict[str, Any]:
        ...
        計算アルゴリズム:
            1. 総対戦数を計算（0の場合は全ての値を0で返す）
            2. 勝敗を is_win フラグで分類（True=勝利、False=敗北）
            3. 先攻/後攻を is_going_first フラグで分類（True=先攻、False=後攻）
            4. 各カテゴリーの勝率を計算:
               - 総合勝率 = 勝利数 / 総対戦数 × 100
        ...
        # 先攻・後攻の対戦をそれぞれ抽出
        # is_going_first: True = 先攻, False = 後攻
        ...
        # コイントス勝率を計算（won_coin_toss: True = 勝ち）
