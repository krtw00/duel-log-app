...
class Duel(Base):
    ...
    opponentDeck_id = Column(Integer, ForeignKey("decks.id"), nullable=False)  # 相手のデッキ

    # 対戦結果フィールド
    result = Column(Boolean, nullable=False)  # 勝敗: True = 勝ち, False = 負け
    ...
    # ターン順・コイントス関連フィールド
    coin = Column(Boolean, nullable=False)  # コイントス結果: True = 勝ち, False = 負け
    first_or_second = Column(Boolean, nullable=False)  # ターン順: True = 先攻, False = 後攻
