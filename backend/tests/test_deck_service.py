...
class TestDeckService:
    ...
    def test_create_deck(self, db_session, test_user):
        ...
        deck = deck_service.create_user_deck(db_session, user_id=test_user.id, deck_in=deck_in)
    ...
    def test_get_deck_by_id(self, db_session, test_user):
        ...
        created_deck = deck_service.create_user_deck(db_session, user_id=test_user.id, deck_in=deck_in)
    ...
    def test_get_user_decks_filtered_by_opponent(self, db_session, test_user):
        ...
        deck_service.create_user_deck(db_session, user_id=test_user.id, deck_in=opponent_deck)
    ...
    def test_create_duplicate_deck(self, db_session, test_user):
        ...
        deck_service.create_user_deck(db_session, user_id=test_user.id, deck_in=deck_in1)
        
        # 同じ名前のデッキを作成しようとする
        deck_in2 = DeckCreate(name="Duplicate Deck", is_opponent=False)
        
        with pytest.raises(ValueError, match="同じ名前の"):
            deck_service.create_user_deck(db_session, user_id=test_user.id, deck_in=deck_in2)
    ...
    def test_update_to_duplicate_name(self, db_session, test_user):
        ...
        created_deck1 = deck_service.create_user_deck(db_session, user_id=test_user.id, deck_in=deck1)
        created_deck2 = deck_service.create_user_deck(db_session, user_id=test_user.id, deck_in=deck2)
