...
def create_matchup_test_data(db: Session, user: User):
    ...
    my_deck1 = deck_service.get_or_create(db, user_id=user.id, name="MyDeck1", is_opponent=False)
    my_deck2 = deck_service.get_or_create(db, user_id=user.id, name="MyDeck2", is_opponent=False)
...
class TestMatchupService:
    ...
    def test_get_matchup_chart(self, db_session: Session, test_user: User):
        ...
        mydeck1_vs_oppdeck1 = next((d for d in chart_data if d["deck_name"] == "MyDeck1" and d["opponent_deck_name"] == "OppDeck1"), None)
        ...
        mydeck1_vs_oppdeck2 = next((d for d in chart_data if d["deck_name"] == "MyDeck1" and d["opponent_deck_name"] == "OppDeck2"), None)
        ...
        mydeck2_vs_oppdeck1 = next((d for d in chart_data if d["deck_name"] == "MyDeck2" and d["opponent_deck_name"] == "OppDeck1"), None)
