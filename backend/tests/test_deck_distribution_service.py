...
class TestDeckDistributionService:
    """DeckDistributionServiceのテストクラス"""

    def test_get_deck_distribution_monthly_no_range(
        ...
        opp_deck1_stats = next(
            (item for item in distribution if item["deck_name"] == "OppDeck1"), None
        )
        opp_deck2_stats = next(
            (item for item in distribution if item["deck_name"] == "OppDeck2"), None
        )
    ...
    def test_get_deck_distribution_monthly_with_range(
        self, db_session: Session, test_user: User
    ):
    ...
    def test_get_deck_distribution_recent_with_limit(
        self, db_session: Session, test_user: User
    ):
        ...
        distribution = deck_distribution_service.get_deck_distribution_recent(
            db=db_session, user_id=test_user.id, limit=5
        )
    ...
    def test_get_deck_distribution_recent_with_range(
        self, db_session: Session, test_user: User
    ):
        ...
        distribution = deck_distribution_service.get_deck_distribution_recent(
            db=db_session, user_id=test_user.id, range_start=2, range_end=5
        )
    ...
    def test_get_deck_distribution_monthly_with_my_deck_filter(
        self, db_session: Session, test_user: User
    ):
        """自分のデッキで絞り込んだ場合にパーセンテージが正しく計算されることをテスト"""
        create_test_data(db_session, test_user)
        my_deck1 = deck_service.get_by_name(
            db_session, user_id=test_user.id, name="MyDeck1", is_opponent=False
        )
    ...
    def test_get_deck_distribution_recent_with_my_deck_filter(
        self, db_session: Session, test_user: User
    ):
        """直近分布でも自分のデッキ絞り込みが反映されることをテスト"""
        create_test_data(db_session, test_user)
        my_deck2 = deck_service.get_by_name(
            db_session, user_id=test_user.id, name="MyDeck2", is_opponent=False
        )
