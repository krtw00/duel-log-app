...
テンプレ画像（`VICTORY` / `LOSE`、必要なら「選択してください」文言）は、運用で差し替えが必要になる可能性があります。

推奨:

- 正規化後（幅1280相当）に合わせたテンプレを用意する
- 前処理（エッジ化）後のテンプレも併せて保持し、実行時のコストを下げる
- フロント側で読み込む場合は `public/screen-analysis/` に配置し、URLで参照する
  - `public/screen-analysis/turn-choice.png`
  - `public/screen-analysis/result-win.png`
  - `public/screen-analysis/result-lose.png`
