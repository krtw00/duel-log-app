...
本アプリケーションは、日本語、英語、韓国語の多言語対応を計画しています。

- **対応予定言語**: 日本語（現在）、英語、韓国語
- **優先順位**: 英語対応を優先的に実装予定
- **技術**: vue-i18n v9.x を使用予定

詳細な設計については [多言語対応設計書](docs/design/internationalization.md) を参照してください。
...
将来的にはアプリ内から直接バグ報告や機能要望を送信できるフィードバック機能の実装を計画しています:

- **ヘルプメニュー**: AppBarの「?」アイコンからアクセス可能
- **バグ報告フォーム**: 問題の種類、再現手順、環境情報を自動収集
- **機能要望フォーム**: 新機能のリクエストを簡単に送信
- **お問い合わせ**: 開発者への直接連絡手段

詳細な設計については [フィードバック・連絡先機能設計書](docs/design/feedback-and-contact.md) を参照してください。

### セットアップガイド

GitHub Issuesの詳細なセットアップ方法については、[バグトラッキングシステム セットアップガイド](docs/bug-tracking-setup.md) を参照してください。
